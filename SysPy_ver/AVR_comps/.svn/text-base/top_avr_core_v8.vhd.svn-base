...
-- Unused out_en
io_port_out_en(8 to 15) <= (others => '0');
io_port_out(8 to 15) <= (others => (others => '0'));
...
-- Data memory(8-bit)					   
DRAM_Inst:component XDM32Kx8_3 port map(
	                    cp2       => ram_cp2_n,
						ce        => vcc,
...
-- Program memory					   
PM_Inst:component XPM8Kx16_3 port map(
	                  cp2     => ram_cp2_n, 
					  ce      => vcc,
