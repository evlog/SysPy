...
*****************************************************************************
 ...
 Generates block_ram.init file from binary images. Binary images are first 
 converted in hex files using bin2hex.c file, provided with the ORPSoC v2 
 project. bin2hex executable  must exist in the same folder with this script.
...
def bin2init(initFilename):    
       """
       FUNCTION: bin2init(a str)
              a:  binary filename
	...
	#-------------------------------------------------------------------------------
       ...
       #-------------------------------------------------------------------------------
       if (initFilename.find(".bin") == -1):
              raise MyExceptions("*.bin file required")
       #-------------------------------------------------------------------------------

       i = initFilename.find(".bin")

       initFilename = initFilename[:i]

       # Deleting old *.hex and *.bin files
       #-------------------------------------------------------------------------------
       commands.getoutput("rm " + initFilename + ".hex")
       commands.getoutput("rm " + initFilename + ".init")
       #-------------------------------------------------------------------------------

       ## Calling bin2hex executable to convert *.bin file to *.hex
       commands.getoutput("./bin2hex " + initFilename + ".bin 4 > "+ initFilename + ".hex")

       # Opening the *.hex and the *.init file
       #-------------------------------------------------------------------------------
       hexFile = open(initFilename + ".hex", 'r')
       initFile = open(initFilename + ".init", 'w')
       #-------------------------------------------------------------------------------
       ...
       #-------------------------------------------------------------------------------    
       print("File \"" + initFilename + ".init\" generated.")
       hexFile.close()
       initFile.close()
       #-------------------------------------------------------------------------------
